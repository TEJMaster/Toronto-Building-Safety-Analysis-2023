...
For our research, we collected data from the City of Toronto’s website utilizing the "opendatatoronto" package [@Toronto_2023]. We adjusted certain variables and excluded observations with missing values, as several variables were incomplete. A new variable, "AGE" was introduced to represent the building's age, which served as the sole predictor in our simple linear regression analysis.

Apartment Building Standards program was initiated in 2017 to ensure that apartment buildings with at least three stories or 10 units adhere to required maintenance standards. This initiative targets building owners and operators, ensuring compliance through regular inspections.

This dataset provides scores from these evaluations for registered RentSafeTO buildings, where Bylaw Enforcement Officers inspect areas like common spaces, mechanical and security features, parking, and exterior grounds.

In 2023, the evaluation method of RentSafeTO saw significant changes. Now, buildings are assessed at minimum every two years, with the inclusion of 50 different categories in the evaluation, each given a specific weight.

During an evaluation, each category is scored on a scale from one to three, with one being the lowest score and three the highest. Occasionally, a category might receive a score of 0 if it’s not possible to evaluate it due to blockages or refusal of access. If a category is not relevant at the time of evaluation, it's left blank in the dataset.
...
**1. Minimum Residual**: The minimum residual is -69.700, indicating that there is at least one building for which the model's predicted evaluation score was lower than the actual score by approximately 69.700 points. This could be an outlier or a building with exceptional characteristics not captured by the model.

**2. First Quartile Residual**: The first quartile (Q1) of the residuals is -4.440, meaning that 25% of the buildings have evaluation scores that are less than 4.440 points below the model's prediction for their age.

**3. Median Residual**: The median residual is 1.687, suggesting a slight positive bias in the model's predictions, as the median of the error distribution is above zero. This indicates that, on average, the model tends to slightly overestimate the evaluation scores.

**4. Third Quartile Residual**: The third quartile (Q3) is 5.860, indicating that 75% of the buildings have evaluation scores within 5.860 points of the model's predictions or better. This shows that the model provides reasonably accurate predictions for the majority of buildings.

**5. Maximum Residual**: The maximum residual is 15.715, suggesting that there is at least one building for which the model's predicted evaluation score was higher than the actual score by approximately 15.715 points. This could be another outlier or a building that performed exceptionally well.
